...
Read this section first; use the long setter/defaultValue tables only when the
current property type requires them.

1. Read the runtime component and list every `props.attributes[fieldName]` it consumes.
2. Read `commonProperties.json`; reject new custom `fieldName` collisions with common fields.
3. Define only the custom groups/properties that map to real runtime attributes.
4. Derive `supportedEvents` from proven `onX?.(...)` forwarding, not from guesses.
5. Derive `supportedActions` from active contracts in `contract/metadata.ts`; avoid aliases in new configs.
6. If event params are useful, ensure `valueSources.ts` has semantic labels such as `输入值` / `是否选中` / `当前行`.
7. If an action returns data for later actions, ensure `contract/outputSchema.ts` exposes those fields.
8. Run `node scripts/validate-component-config.mjs {Name}` for new configs or `--legacy` for existing legacy configs.
...
```json
{
  "disabledCommonProps": ["common_rotation", "common_shadow"]
}
```
...
Interaction config is not just a pair of string arrays. Each event/action must
match the runtime forwarding behavior, the interaction contracts, and the editor's
semantic value-source selectors.

Current contract sources:

| Source                                                                                                     | Purpose                                                                                |
| ---------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `packages/snify/src/core/interaction/contract/metadata.ts`                                                 | Active/planned event and action metadata. Use this as the strongest source for status. |
| `packages/snify/src/core/interaction/contract/registry.ts`                                                 | Runtime handler registry and normalized contract exports.                              |
| `packages/snify/src/core/interaction/contract/outputSchema.ts`                                             | Action output fields shown by the action-result selector.                              |
| `apps/editor/src/designable/designable-react-settings-form/src/widgets/InteractiveSidebar/valueSources.ts` | Editor semantic labels for event params, action outputs, and loop context.             |
...
**Available event contract keys** (verify live in `core/interaction/contract/metadata.ts`):
...
| `submit`     | 提交时   | form      |
| `confirm`    | 确认时   | form      |
| `touchstart` | 触摸开始 | touch     |
| `touchmove`  | 触摸移动 | touch     |
| `touchend`   | 触摸结束 | touch     |
| `longpress`  | 长按时   | touch     |
| `play`       | 播放时   | media     |
| `pause`      | 暂停时   | media     |
...
> `load` is a valid lifecycle contract, but `withCustomWrapper` does not auto-fire it.
> Only expose `load` when the component/runtime has verified mount dispatch.

### Event Parameter Semantics

When `setVariable` uses source type `event`, the editor now shows semantic choices
instead of raw paths. Do not design new configs that require users to type
`0.detail.value` by hand.

The JSON config must be backed by runtime evidence:

| Event pattern                             | Runtime forwarding proof                                                  | Editor semantic option                |
| ----------------------------------------- | ------------------------------------------------------------------------- | ------------------------------------- |
| Button-like `click` / `tap`               | Component only signals a click; no business payload required.             | No ordinary event-param option        |
| Input-like `input` / `change` / `confirm` | Component forwards event-like data with `detail.value` or `target.value`. | `输入值`                              |
| Switch / Checkbox `change`                | Component forwards checked boolean as arg 0.                              | `是否选中`                            |
| TabBar `change`                           | Component forwards selected value as arg 0.                               | `选中值`                              |
| List / Table / Repeater row event         | Component dispatch includes `loopContext: { item, index, key }`.          | `当前行` / `当前行 ID` / `当前行索引` |

Rules:

1. Do not infer event params from `supportedEvents` alone. Read the component source and verify what `onX?.(...)` actually passes.
2. `click` does not imply there is a useful event variable. Button click should normally have an empty event-param picker.
3. Row-level actions must use `loopContext`, not a static array index path and not another component's private variable.
4. If a new component exposes a new semantic event value, update `buildEventPathOptions` and `valueSources.test.ts` in the editor.
5. Raw event object access is compatibility/advanced behavior. It must not be the normal user-facing path for new component work.
...
| `requestData`              | ✅              | 请求数据     | ✅ Active                                               |
| `createRecord`             | ✅              | 新增记录     | ✅ Active                                               |
| `updateRecord`             | ✅              | 更新记录     | ✅ Active                                               |
| `deleteRecord`             | ✅              | 删除记录     | ✅ Active                                               |
...
| `showToast`                | ✅              | 显示轻提示   | ✅ Active                                               |
| `showModal`                | ✅              | 显示模态框   | ✅ Active                                               |
| `showLoading`              | ✅              | 显示加载中   | ✅ Active                                               |
| `hideLoading`              | ✅              | 隐藏加载中   | ✅ Active                                               |
| `showActionSheet`          | ✅              | 显示操作菜单 | ✅ Active                                               |
| `navigateTo`               | ✅              | 跳转页面     | ✅ Active                                               |
| `navigateBack`             | ✅              | 返回上一页   | ✅ Active                                               |
| `redirectTo`               | ✅              | 重定向页面   | ✅ Active                                               |
| `switchTab`                | ✅              | 切换 Tab     | ✅ Active                                               |
| `reLaunch`                 | ✅              | 重启应用     | ✅ Active                                               |
| `copyToClipboard`          | ✅              | 复制内容     | ✅ Active                                               |
| `scanCode`                 | ✅              | 扫码         | ✅ Active                                               |
| `chooseImage`              | ✅              | 选择图片     | ✅ Active                                               |
| `previewImage`             | ✅              | 预览图片     | ✅ Active                                               |
| `makePhoneCall`            | ✅              | 拨打电话     | ✅ Active                                               |
| `getLocation`              | ✅              | 获取位置     | ✅ Active                                               |
...
> ⚠️ Do NOT list planned actions (`request`, `setState`, `setHideOrShow`, etc.).
> They have no complete runtime/editor contract and will silently disappear or fail.

> ⚠️ **MetadataService silently filters invalid event/action keys.** If you list an event or action
> that doesn't exist in the contract metadata/registry, it won't error — the interaction panel will
> simply show no options for it. Always verify keys against the contract sources above.

### Action Output Semantics

Actions that return reusable data must be selectable by later actions in the same
event/scenario chain. The editor's action-result selector only shows previous
actions with an output schema.

Current output fields:

| Action         | Ordinary fields                       | System fields for condition/debug                 |
| -------------- | ------------------------------------- | ------------------------------------------------- |
| `requestData`  | `records`, `total`, `params`, `error` | `system.success`, `system.status`, `system.error` |
| `createRecord` | `record`, `error`                     | `system.success`, `system.status`, `system.error` |
| `updateRecord` | `record`, `error`                     | `system.success`, `system.status`, `system.error` |
| `deleteRecord` | `deletedId`, `error`                  | `system.success`, `system.status`, `system.error` |

Rules:

1. Ordinary `setVariable -> 动作结果` hides `system.*` fields by default.
2. Condition/scenario UI may expose `success`, `status`, and `error`.
3. If a new action returns data that users should bind, update `outputSchema.ts`, editor labels in `valueSources.ts`, and tests.
4. The standard MVP flow for list/table data is:
   `requestData -> action output records -> setVariable page.list.records -> List/Table binding`.
...
4. **Contract exists**: Verify the event/action key exists in `contract/metadata.ts` and `contract/registry.ts`
5. **Event params**: Verify the runtime payload has a semantic picker option or intentionally has no business params
6. **Action outputs**: For data-returning actions, verify `contract/outputSchema.ts` and editor value-source tests are updated
...
Use the project validation script that **dynamically reads** `contract/metadata.ts`, `SETTER_MAP`, and `commonProperties.json`:
...
- Event keys exist in `eventContracts` (from `metadata.ts`)
...
| `core/interaction/contract/metadata.ts`          | Active/planned event/action contract metadata                                        |
| `core/interaction/contract/registry.ts`          | Runtime handler registry and normalized contract exports                             |
| `core/interaction/contract/outputSchema.ts`      | Action output fields for the action-result selector                                  |
...
| `scripts/validate-component-config.mjs`          | **V9 validation script** — dynamically reads metadata, SETTER_MAP, common fieldNames |
